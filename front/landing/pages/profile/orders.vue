  ...
  <div class="table-responsive">
    <table class="table align-middle">
      ...
      <tr v-for="order in data.orders" :key="order.id">
        ...
        <td>
          <span
              :class="{'text-success':order.payment_status=='موفق', 'text-danger':order.payment_status=='ناموفق'}">{{ order.payment_status }}</span>
        </td>
        <td>{{ order.amount }}</td>
                ...
                <div class="modal-body">
                  <table class="table align-middle">
                    ...
                    <tr v-for="product in order.order_items" :key="product.id">
                      <th>
                        <img src="/images/preloader.png" v-img="product.product_primary_image" width="80" alt="">
                      </th>
    ...
    <nav class="d-flex justify-content-center mt-5">
      <ul class="pagination">
        <li v-for="(link,index) in data.meta.links.slice(1,-1)" :key="index" class="page-item" :class=" {active : link.active}" >
          <button @click="paginate(link.label)" class="page-link" >{{link.label}}</button></li>
      </ul>
    </nav>
  </div>
